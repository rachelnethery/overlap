rn_2spl<-function(datall,RO,nburn=10000,nsamp=5000){
  ...
  p_spl1<-7+(ncol(datall)-3)
  p_spl0<-7+(ncol(datall)-3)
  ...
  for (i in 1:(nburn+nsamp)){
    ...
    ## update Y1s ##
    Y1s[which(datov$x==0)]<-temp$test[which(datov$x==0)]
    Y0s[which(datov$x==1)]<-temp$test[which(datov$x==1)]
    ...
    if (nrow(datno1)>0){
      ...
      if (ncol(datov)<=3){
        ...
        X_spl<-as.matrix(cbind(1,bs_ps,bs_y1s,datov[,4:ncol(datov)]))
      }
      ...
      #delta_star[which(datno$x==1)]<-rnorm(n=nrow(datno1),mean=Eppd,sd=sqrt(sigma_spl1+exp(ROdist[which(datno$x==1)])*(max(delta)-min(delta))))
    ...
    if (nrow(datno0)>0){
      ...
      if (ncol(datov)<=3){
        ...
        X_spl<-as.matrix(cbind(1,bs_ps,bs_y0s,datov[,4:ncol(datov)]))
      }
      ...
      #delta_star[which(datno$x==0)]<-rnorm(n=nrow(datno0),mean=Eppd,sd=sqrt(sigma_spl0+exp(ROdist[which(datno$x==0)])*(max(delta)-min(delta))))
