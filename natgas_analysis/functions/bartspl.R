bartspl<-function(datall,RO,nburn=10000,nsamp=5000){
  ...
  for (i in 1:(nburn+nsamp)){
    ...
    bs_ps<-rcspline.eval(datov_sp$ps,knots=quantile(datov_sp$ps,probs=c(.1,.33,.66,.9)),inclx = T)
    ...
    if (nrow(datno1)>0){
      ...
      ######################################################################
      bs_ps_star<-rcspline.eval(datno1$ps,knots=quantile(datov_sp$ps,probs=c(.1,.33,.66,.9)),inclx = T)
    ...
    if (nrow(datno0)>0){
      ...
      ## 3. draw from the posterior predictive for the non-overlap region ##
      ######################################################################
      bs_ps_star<-rcspline.eval(datno0$ps,knots=quantile(datov_sp$ps,probs=c(.1,.33,.66,.9)),inclx = T)
