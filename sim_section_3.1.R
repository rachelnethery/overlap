#####################################################################
## THIS CODE IMPLEMENTS THE SIMULATION IN SECTION 3.1 OF THE PAPER ##
#####################################################################
...
while (simscomplete<=(reps-1)){
  ...
  ## skip over datasets that have too much non-overlap in the left tail (RNs with >10 PS's) ##
  if (ll<N & sum(1-order_RO[1:ll])<=10){
    ...
    Y0<-.85*u2+0.05*u2^2+2
    Y1<-(1/(1+exp(-((u2*8)-1))))+(0.25*u1)
    ce_true<-Y1-Y0
