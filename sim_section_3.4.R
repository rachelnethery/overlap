#####################################################################
## THIS CODE IMPLEMENTS THE SIMULATION IN SECTION 3.4 OF THE PAPER ##
#####################################################################
...
kk<-args[5] #string "easy" for simulation 3.4A and string "hard" for simulation 3.4B#
...
for (gg in 1:length(mu)){
  ...
  for (hh in 1:3){
    if (hh==1) RO<-iw_overlap(ps=u,E=x,a=.05*(max(u)-min(u)),b=10)
    else if (hh==2) RO<-iw_overlap(ps=u,E=x,a=.1*(max(u)-min(u)),b=10)
    else if (hh==3) RO<-iw_overlap(ps=u,E=x,a=.15*(max(u)-min(u)),b=3)
