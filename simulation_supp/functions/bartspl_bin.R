bartspl_bin<-function(datall,RO,nburn=10000,nsamp=5000){
  
  ## this function implements BART+SPL method (for binary outcomes)
  ...
  p_spl1<-6+(ncol(datall)-3)
  p_spl0<-6+(ncol(datall)-3)
  ...
  for (i in 1:(nburn+nsamp)){
    ...
    ## arcsine transform the estimated causal effects in the RO ##
    delta_old<-delta
    delta<-asin(delta)
    ...
    Y1s[which(datov_sp$x==0)]<-foosp[which(datov_sp$x==0)]>.5
    Y0s[which(datov_sp$x==1)]<-foosp[which(datov_sp$x==1)]>.5
    ...
    if (nrow(datno1)>0){
      ...
      if (ncol(datov_sp)<=3){
        ...
        X_spl<-as.matrix(cbind(1,bs_ps,Y1s,datov_sp[,4:ncol(datov_sp)]))
      }
    ...
    if (nrow(datno0)>0){
      ...
      if (ncol(datov_sp)<=3){
        ...
        X_spl<-as.matrix(cbind(1,bs_ps,Y0s,datov_sp[,4:ncol(datov_sp)]))
      }
